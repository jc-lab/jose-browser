...
export function compactVerify(jose: JoseType, jws: string | Uint8Array, key: KeyLike | Uint8Array, options?: VerifyOptions): Promise<CompactVerifyResult>;
export function compactVerify(jose: JoseType, jws: string | Uint8Array, getKey: CompactVerifyGetKey, options?: VerifyOptions): Promise<CompactVerifyResult & ResolvedKey>;
export function compactVerify(jose: JoseType, jws: string | Uint8Array, key: KeyLike | Uint8Array | CompactVerifyGetKey, options?: VerifyOptions): Promise<CompactVerifyResult | (CompactVerifyResult & ResolvedKey)> {
  ...
  return flattenedVerify(
    ...
    <Parameters<typeof flattenedVerify>[2]>key,
    options,
  )
    .then((verified) => {
...
export function makeCompactSign(jose: JoseType): typeof CompactSign {
  class NewCompactSign extends jose.CompactSign {
    private readonly _flattenedImpl: FlattenedSign;

    constructor(payload: Uint8Array) {
      super(payload);
      this._flattenedImpl = new jose.FlattenedSign(payload);
    }
